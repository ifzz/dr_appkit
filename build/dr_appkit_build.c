...
int main()
{
    ...
    char exePath[DRFS_MAX_PATH];
    if (!dr_get_executable_path(exePath, sizeof(exePath))) {
        return -1;
    }

    char exeDirPath[DRFS_MAX_PATH];
    drpath_copy_base_path(exePath, exeDirPath, sizeof(exeDirPath));
    
    drfs_context* pVFS = drfs_create_context();
    drfs_set_base_write_directory(pVFS, exeDirPath);
    drfs_add_base_directory(pVFS, exeDirPath);

    drfs_file* pFile = drfs_open(pVFS, "../source/ak_autogen.c", DRFS_WRITE | DRFS_TRUNCATE, 0);
    if (pFile == NULL) {
        return -2;
    }

    drfs_write_line(pFile, "// This file was generated by a tool. Do not modify.\n");
    ...
    // Cross.
    char* svg = drfs_open_and_read_text_file(pVFS, "../resources/cross.svg", NULL);
    ...
    if (g_AKImage_Cross_16x16 != NULL) {
        drfs_write_string(pFile, g_AKImage_Cross_16x16);
        drfs_write_string(pFile, "\n\n");
    ...
    if (g_AKImage_Cross_24x24 != NULL) {
        drfs_write_string(pFile, g_AKImage_Cross_24x24);
        drfs_write_string(pFile, "\n\n");
    ...
    if (g_AKImage_Cross_32x32 != NULL) {
        drfs_write_string(pFile, g_AKImage_Cross_32x32);
        drfs_write_string(pFile, "\n\n");
